...
//************* Start Initial Sequence **********//
	...
	WriteComm(0xB0);   ///命令保护，设置用户和厂家命令都有效；
	WriteData(0x3F);
	WriteData(0x3F);
	for(i=500; i>0; i--);

	WriteComm(0xB3);   ///帧空间获取命令
	WriteData(0x02);   ///WEMOD设置为1，连续数据覆盖窗口；
	WriteData(0x00);
	WriteData(0x00);
	WriteData(0x00);
	WriteData(0x00);
	...
	WriteComm( 0xC0 ); ///Panel Driving Setting
	WriteData( 0x33);	//03
	WriteData( 0x4F);
	WriteData( 0x00);
	WriteData( 0x10);
...
void WriteData(uint i)    ///写入数据，只有低8位有效传入；
{
	TFT_WR = 1;	 		  //初始化WR
	TFT_CS = 0;			  //打开片选
...
void WriteComm(uint i)     ///写入命令，只有低8位有效传入；
{
	TFT_WR = 1;	 		  //初始化WR
	TFT_CS = 0;			  //打开片选
